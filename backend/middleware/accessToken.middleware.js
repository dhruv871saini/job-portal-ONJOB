...
const accessTokenMiddleware= async (req ,res ,next)=>{
    ...
    try {
        if (!authHeader){
        return newResponse(401,"Unauthorized : token missing")
    }
    const token =authHeader.split(" ")[1]
    const decode = jwt.verify(token,process.env.SECRET_KEY)
    const user = await employer.findById(decode.id).select("comapanyPassword") 
    if(!user){
        return newResponse(401,"Unauthorized user not found")
    ...
    next()
        ...
        return newResponse(500,"server error with middlware",error)
    }
}
